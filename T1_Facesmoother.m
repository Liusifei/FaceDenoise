...
img = atan(img*1.5)/0.9;
dimg = T1_Avgsmoother(img);
edge_bw = edge(rgb2gray(img),'canny',[0.01,0.10]);
...
rut_im = repmat(w_im,[1,1,n]) .* double(ref_im) + (1-repmat(w_im,[1,1,n])) .* double(dst_im);
end
%% T1_Avgsmoother.m

function RI = T1_Avgsmoother(I)
H = fspecial('average',2);
fI = imfilter(I,H,'replicate');
H = fspecial('average',25);
ffI = imfilter(I,H,'replicate');
RI = ffI + (I-fI);
end
%% T2_MultiDoG.m
